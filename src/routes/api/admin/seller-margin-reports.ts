...
type SellerMapping = {
  userId: string;
  accountName: string;
  source: "account_code" | "previous_report";
};
...
const NUMERIC_FIELDS = [
  ...
  "margemBruta",
  "margemAditivos",
  "margemAditivosPercentual",
  "margemSacarias",
  "margemSacariasPercentual",
  "margemBalcao",
  "margemBalcaoPercentual",
] as const;
...
function readNumber(value: unknown, label: string): number {
  ...
  if (!Number.isFinite(parsed) || Math.abs(parsed) > 100000000000000) {
    throw new Error(label + " inválido.");
  }
...
function safeSourceFile(value: unknown): string {
...
function namesLookCompatible(reportName: string, accountName: string): boolean {
  const report = normalizeName(reportName);
...
async function authorizeAdmin(request: Request) {
  ...
  const { data: role, error } = await authorization.supabaseAdmin
    .from("user_roles")
    .select("id")
  ...
  if (error || !role) {
    return {
      response: errorResponse("Acesso restrito ao administrativo.", 403),
    } as const;
  }
...
function validateRows(value: unknown): ValidatedReportRow[] {
  if (!Array.isArray(value) || value.length === 0 || value.length > 500) {
    throw new Error("O relatório precisa conter entre 1 e 500 vendedores.");
  }
  ...
  return value.map((entry, index) => {
    ...
    if (!/^\d{1,10}$/.test(rawCode)) {
      throw new Error("Código inválido na linha " + (index + 1) + ".");
    }

    const code = normalizeSellerCode(rawCode);
    if (seenCodes.has(code)) {
      throw new Error("O código " + code + " aparece mais de uma vez.");
    }
    seenCodes.add(code);

    const name = readText(row.name, "Vendedor da linha " + (index + 1), 120);
    const result = { code, name } as ValidatedReportRow;

    for (const field of NUMERIC_FIELDS) {
      (result as Record<string, unknown>)[field] = readNumber(
        row[field],
        field + " da linha " + (index + 1),
      );
    }
    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        const { data, error } = await authorization.supabaseAdmin
          ...
          .select(
            "report_year,report_month,period_start,period_end,report_seller_code,report_seller_name,total_venda,margem_percentual,tonelagem,seller_user_id,source_file,updated_at",
          )
          .order("period_start", { ascending: false })
          .order("report_seller_code", { ascending: true })
          .limit(3000);
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          return response({ error: "Dados da importação inválidos." }, { status: 400 });
        }

        let periodStart: string;
        let periodEnd: string;
        let rows: ValidatedReportRow[];
        ...
        try {
          ...
          return response(
            { error: error instanceof Error ? error.message : "Dados da importação inválidos." },
            { status: 400 },
          );
        }

        const reportYear = Number(periodStart.slice(0, 4));
        const reportMonth = Number(periodStart.slice(5, 7));
        ...
        for (const user of users) {
          ...
          const code = normalizeSellerCode(codeValue);
          const current = sellerByCode.get(code);
          if (current && current.userId !== identity.userId) {
            return response(
              { error: "O código de vendedor " + code + " está vinculado a mais de uma conta." },
              { status: 409 },
            );
          }

          sellerByCode.set(code, {
            userId: identity.userId,
            accountName: identity.name || accountName(user, "Vendedor"),
            source: "account_code",
          });
        }

        const { data: existingReports, error: existingError } = await authorization.supabaseAdmin
          .from("seller_monthly_margin_reports")
          .select("report_seller_code,seller_user_id")
        ...
        const records = rows.map((row) => {
          ...
          if (seller) {
            ...
            unlinkedRows.push({ code: row.code, name: row.name });
          }

          return {
            seller_user_id: seller?.userId ?? null,
            report_year: reportYear,
            report_month: reportMonth,
            period_start: periodStart,
            period_end: periodEnd,
            report_seller_code: row.code,
            report_seller_name: row.name,
            total_venda: row.totalVenda,
            devolucao: row.devolucao,
            aditivos: row.aditivos,
            sacarias: row.sacarias,
            balcao: row.balcao,
            total_custo: row.totalCusto,
            margem_percentual: row.margemPercentual,
            comissao_representante: row.comissaoRepresentante,
            tonelagem: row.tonelagem,
            margem_bruta: row.margemBruta,
            margem_aditivos: row.margemAditivos,
            margem_aditivos_percentual: row.margemAditivosPercentual,
            margem_sacarias: row.margemSacarias,
            margem_sacarias_percentual: row.margemSacariasPercentual,
            margem_balcao: row.margemBalcao,
            margem_balcao_percentual: row.margemBalcaoPercentual,
            source_file: sourceFile,
            updated_at: new Date().toISOString(),
          };
        });

        const { error: upsertError } = await authorization.supabaseAdmin
          .from("seller_monthly_margin_reports")
          .upsert(records, {
            onConflict: "report_year,report_month,report_seller_code",
          });
        ...
        return response({
          ...
          linkedRows: linked,
          unlinkedRows,
          updatedRows: rows.filter((row) => previousByCode.has(row.code)).length,
          insertedRows: rows.filter((row) => !previousByCode.has(row.code)).length,
          warnings,
          mappings: rows.map((row) => {
            ...
            return {
              code: row.code,
              name: row.name,
              linked: Boolean(current || previousUserId),
              accountName: current?.accountName ?? null,
              source: current?.source ?? (previousUserId ? "previous_report" : null),
            };
          }),
        });
      },
