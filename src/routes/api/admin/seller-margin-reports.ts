...
const NUMERIC_FIELDS = [
  "totalVenda", "devolucao", "aditivos", "sacarias", "balcao", "totalCusto",
  "margemPercentual", "comissaoRepresentante", "tonelagem", "margemBruta",
  "margemAditivos", "margemAditivosPercentual", "margemSacarias",
  "margemSacariasPercentual", "margemBalcao", "margemBalcaoPercentual",
] as const;
...
function readText(value: unknown, label: string, maximumLength: number): string {
  ...
  if (!text || text.length > maximumLength) throw new Error(`${label} inválido.`);
  return text;
}
...
async function authorizeAdmin(request: Request) {
  ...
  const { data: role, error } = await authorization.supabaseAdmin
    .from("user_roles").select("id").eq("user_id", authorization.user.id).eq("role", "admin").limit(1).maybeSingle();
...
function validateRows(value: unknown): ValidatedReportRow[] {
  ...
  return value.map((entry, index) => {
    ...
    if (seenCodes.has(code)) throw new Error(`O código ${code} aparece mais de uma vez.`);
    seenCodes.add(code);
    const result = { code, name: readText(row.name, `Vendedor da linha ${index + 1}`, 120) } as ValidatedReportRow;
    for (const field of NUMERIC_FIELDS) {
      (result as Record<string, unknown>)[field] = readNumber(row[field], `${field} da linha ${index + 1}`);
    }
    return result;
  });
}
      ...
      POST: async ({ request }) => {
        ...
        for (const user of users) {
          ...
          if (current && current.userId !== identity.userId) {
            return response({ error: `O código de vendedor ${code} está vinculado a mais de uma conta.` }, { status: 409 });
          }
        ...
        const { data: existingReports, error: existingError } = await authorization.supabaseAdmin
          .from("seller_monthly_margin_reports")
          .select("report_seller_code,seller_user_id,period_end")
        ...
        const records = rows.map((row) => {
          ...
          if (seller) {
            ...
            if (current && !namesLookCompatible(row.name, current.accountName)) {
              warnings.push(`Código ${row.code}: o PDF traz "${row.name}" e a conta está como "${current.accountName}". A vinculação foi feita pelo código.`);
            }
