...
type PricingCatalog = { name?: string | null; slug?: string | null };
...
export type PricingProduct = {
  ...
  catalogs?: PricingCatalog | PricingCatalog[] | null;
};
...
function productCategories(product: PricingProduct): PricingCatalog[] {
  if (!product.catalogs) return [];
  return Array.isArray(product.catalogs) ? product.catalogs : [product.catalogs];
}

function isExemptCategory(product: PricingProduct): boolean {
  return productCategories(product).some((category) =>
    [category.name, category.slug].some(
      (value) => value != null && EXEMPT_CATEGORIES.has(categoryKey(value)),
    ),
  );
}

export function consumerTaxRate(product: PricingProduct, accountType = "cliente"): number {
  if (accountType === "produtor" || isExemptCategory(product)) return 0;
...
export function isOnSaleForAccount(product: PricingProduct, type = "cliente"): boolean {
  return Boolean(
    product.on_sale &&
      product.sale_producer_price != null &&
...
export function pixPriceForAccount(product: PricingProduct, type = "cliente"): number | null {
  const base =
    product.on_sale && product.sale_producer_pix_price != null
      ? product.sale_producer_pix_price
      : product.producer_pix_price;
