...
type CustomerRow = {
  id: string; codigo: string; cliente: string; cidade: string | null; uf: string | null;
  telefone: string | null; celular: string | null; email: string | null; ultima_compra: string | null;
  compra_ano: number | string | null; compra_ano_anterior: number | string | null;
  abc_na_carteira_atual: boolean; vendedor_codigo: string | null; vendedor_nome: string | null;
};

type SaleRequestRow = {
  id: string; seller_user_id: string; seller_record_id: string | null; seller_code: string; seller_name: string;
  customer_code: string; customer_name: string; sale_notes: string; sale_value: number | string; status: string; created_at: string;
};

type Totals = {
  total_venda: number; devolucao: number; aditivos: number; sacarias: number; balcao: number; total_custo: number;
  comissao_representante: number; tonelagem: number; margem_bruta: number; margem_aditivos: number; margem_sacarias: number; margem_balcao: number;
};

const TOTAL_KEYS: Array<keyof Totals> = ["total_venda", "devolucao", "aditivos", "sacarias", "balcao", "total_custo", "comissao_representante", "tonelagem", "margem_bruta", "margem_aditivos", "margem_sacarias", "margem_balcao"];

function normalizeCode(value: unknown) { const code = String(value ?? "").trim(); return code.replace(/^0+(?=\d)/, "") || (code ? "0" : ""); }
function codeVariants(value: string) { const raw = value.trim(); const normalized = normalizeCode(raw); return [...new Set([raw, normalized, normalized.padStart(3, "0")].filter(Boolean))]; }
function n(value: unknown) { const parsed = Number(value ?? 0); return Number.isFinite(parsed) ? parsed : 0; }
function emptyTotals(): Totals { return { total_venda: 0, devolucao: 0, aditivos: 0, sacarias: 0, balcao: 0, total_custo: 0, comissao_representante: 0, tonelagem: 0, margem_bruta: 0, margem_aditivos: 0, margem_sacarias: 0, margem_balcao: 0 }; }
function rowTotals(row: MarginRow): Totals { const result = emptyTotals(); for (const key of TOTAL_KEYS) result[key] = n(row[key]); return result; }
function add(target: Totals, source: Totals) { for (const key of TOTAL_KEYS) target[key] += source[key]; }
function subtract(current: Totals, previous: Totals) { const result = emptyTotals(); for (const key of TOTAL_KEYS) result[key] = current[key] - previous[key]; return result; }
function withMargin(totals: Totals) { return { ...totals, margem_percentual: totals.total_venda ? (totals.margem_bruta / totals.total_venda) * 100 : 0 }; }
function isoDate(date: Date) { return date.toISOString().slice(0, 10); }
function parseDate(value: string) { return new Date(`${value}T12:00:00Z`); }
function shiftDays(value: string, days: number) { const date = parseDate(value); date.setUTCDate(date.getUTCDate() + days); return isoDate(date); }
function daysInclusive(from: string, to: string) { return Math.max(1, Math.round((parseDate(to).getTime() - parseDate(from).getTime()) / 86_400_000) + 1); }
function validIso(value: string | null) { if (!value || !/^\d{4}-\d{2}-\d{2}$/.test(value)) return null; const date = parseDate(value); return Number.isFinite(date.getTime()) && isoDate(date) === value ? value : null; }
function monthBounds(year: number, month: number) { return { from: `${year}-${String(month).padStart(2, "0")}-01`, to: isoDate(new Date(Date.UTC(year, month, 0, 12))) }; }
...
function previousPeriod(from: string, to: string, preset: string | null) {
  ...
  if (preset === "year") { const year = Number(from.slice(0, 4)) - 1; return { from: `${year}-01-01`, to: `${year}-${to.slice(5)}` }; }
  if (preset === "month") {
    const source = parseDate(from); source.setUTCMonth(source.getUTCMonth() - 1);
    const previousFrom = `${source.getUTCFullYear()}-${String(source.getUTCMonth() + 1).padStart(2, "0")}-01`;
    const elapsedDays = daysInclusive(from, to); const previousMonthEnd = monthBounds(source.getUTCFullYear(), source.getUTCMonth() + 1).to;
  ...
  const length = daysInclusive(from, to); const previousTo = shiftDays(from, -1);
...
function aggregateRange(rows: MarginRow[], from: string, to: string) {
  ...
  for (const row of rows) { const key = `${normalizeCode(row.report_seller_code)}:${row.report_year}-${String(row.report_month).padStart(2, "0")}`; groups.set(key, [...(groups.get(key) ?? []), row]); }
  const totals = emptyTotals(); let hasData = false; let partialWithoutBaseline = false;
  for (const group of groups.values()) {
    group.sort((a, b) => a.period_end.localeCompare(b.period_end)); const sample = group[0]; const bounds = monthBounds(sample.report_year, sample.report_month);
    if (bounds.to < from || bounds.from > to) continue;
    const effectiveFrom = from > bounds.from ? from : bounds.from; const effectiveTo = to < bounds.to ? to : bounds.to;
    const ending = [...group].reverse().find((row) => row.period_end <= effectiveTo); if (!ending) continue;
    ...
    if (effectiveFrom > ending.period_start) {
      ...
      if (baseline) contribution = subtract(contribution, rowTotals(baseline)); else partialWithoutBaseline = true;
    }
    add(totals, contribution); hasData = true;
  }
  return { totals: withMargin(totals), hasData, partialWithoutBaseline };
}

function pct(current: number, previous: number): number | null { if (Math.abs(previous) < 0.000001) return Math.abs(current) < 0.000001 ? 0 : null; return ((current - previous) / Math.abs(previous)) * 100; }
function comparisons(current: ReturnType<typeof withMargin>, previous: ReturnType<typeof withMargin>) { const result: Record<string, number | null> = {}; for (const key of [...TOTAL_KEYS, "margem_percentual"] as const) result[key] = pct(current[key], previous[key]); return result; }
function latestReport(rows: MarginRow[], from: string, to: string) { return rows.filter((row) => row.period_end >= from && row.period_start <= to).sort((a, b) => b.period_end.localeCompare(a.period_end) || b.updated_at.localeCompare(a.updated_at))[0] ?? null; }
...
export async function buildAdminSalesStatistics(supabaseAdmin: any, request: Request) {
  ...
  return {
    sellerCodeMissing: false, seller: seller ? { id: seller.id, name: seller.name, code: sellerCode } : { id: null, name: "DuKamp", code: null },
